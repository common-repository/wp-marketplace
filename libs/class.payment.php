...
class Payment{
    ...
    function InitiateProcessor($MethodID){
        global $wpdb;
        $sql = "select * from {$wpdb->prefix}mp_payment_methods where id='$MethodID' or class_name='$MethodID'";        
        $Method = $wpdb->get_row($sql,ARRAY_A);                
        $MethodClass = $Method['class_name'];        
        include(dirname(__FILE__)."/{$MethodClass}/class.{$MethodClass}.php");        
